...
Step 5: Per exclusivity policy, emit auto-generated Contradict Actions
        between H_i pairs (see §5).
```
...
When `exclusivity != "none"` and there are 2+ hypotheses, the Likelihood lowering emits `Contradict` (or `Exclusive` / a `Decompose`-helped `Disjunction`) Actions for hypothesis pairs.

These are **independent `Structural.Contradict` Actions**, not nested children of the `Likelihood` Action. They carry an audit marker:
...
A reviewer auditing `likelihood(...)` can grep for `metadata["auto_generated_by"] == "likelihood:<this_label>"` and see the implied exclusivity commitments. This is more transparent than #523's compile-time-only metadata trail because the commitments are real Action records, not just markers on operators.
...
| Auto-generated CONTRADICTION | IR-level operator with metadata trail | Real `Structural.Contradict` Action with `auto_generated_by` marker |
...
What is **not** changing: the IR contract (no new node/factor/operator types), BP semantics, distribution backend protocol, gaia check rules, or the Mendel golden test.
...
cmp = bayes.likelihood(
    ...
    model=M_3to1,
    against=[M_null],
    rationale="3:1 vs 1:1 segregation comparison; pairwise contradiction (default)",
)

# cmp is a Claim. Its content is approximately:
#   "Given F2 dominant count k=295 of n=395, H_3to1 is preferred over H_null (BF ≈ 49)."
...
Auto-generated Contradict between H_3to1 and H_null is appended to the package as an independent Action with `metadata["auto_generated_by"] = "likelihood:<cmp.label>"`.

---

## 9. Roles Registration

PR #524 added `gaia/lang/runtime/roles.py` — a projection that, given the package action graph, returns `RoleOccurrence(claim, role, action, ...)` tuples for every Claim referenced as a typed field on any Action. The projection is consumed by prior policies, `gaia check --hole`, and review manifests.
...
- Tests: distribution unit tests (Milestone A), Action construction tests, lowering tests, Mendel golden integration test.
...
2. `from gaia.lang import bayes` exposes `bayes.model`, `bayes.likelihood`, and the distribution literals.
...
7. `roles_for_package(pkg)` surfaces all six Bayes role names from §9 for the canonical Mendel package.
8. The Mendel golden pipeline (per #523 §7.4) passes with posterior odds in the corrected exhaustive range.
9. Auto-generated `Structural.Contradict` Actions appear in the package action list with `metadata["auto_generated_by"]` markers.
10. PR #506 is closed and `gaia.lang.dsl.evidence_verb` is removed.
11. `docs/foundations/gaia-lang/bayes.md` exists and reflects this revision (helper-Claim shape, `bayes.model` verb, multi-H at Likelihood).
12. No new `FactorType` in `gaia/bp/factor_graph.py`; no new `OperatorType` in `gaia/ir/operator.py`; no `gaia.stats` shim.
